...
final class TemplateLauncher {
    ...
    func apply(_ layouts: [MonitorLayout], excludedBundleIDs: Set<String>) {
        ...
        if inFlight != nil {
            ...
            launchDeadline = nil
            inFlight = nil
            NSLog("Ferrite: superseding an in-flight template launch")
        }
        ...
        for bundleID in multi.appsToLaunch {
            guard let url = NSWorkspace.shared
                ...
                NSLog("Ferrite: no app found for %@", bundleID)
                continue
            }
    ...
    private func reportMissing() {
        ...
        restoreStacking(multi: flight.multi)
        NSLog("Ferrite: template apps failed to launch: %@", missing.joined(separator: ", "))
