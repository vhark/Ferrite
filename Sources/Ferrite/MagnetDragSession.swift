...
final class MagnetDragSession {
    ...
    /// leave in.
    static let trace = ProcessInfo.processInfo.environment["FERRITE_TRACE_DRAG"] == "1"

    private func trace(_ message: @autoclosure () -> String) {
        guard Self.trace else { return }
        NSLog("Ferrite/drag: \(message())")
    }
    ...
    private func openDrag(for event: AppObserver.WindowEvent) {
        ...
        // apps, and the sweep drops Ferrite and every excluded bundle.
    ...
    private func logUnresolved(bundleID: String, windowID: Int) {
        NSLog("Ferrite: snapped windows but did not remember the group — no record "
