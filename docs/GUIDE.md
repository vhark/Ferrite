...
**Maximizing takes a window out of its group.** Double-click a member's title bar (when your system is set to zoom on that gesture rather than minimize or do nothing), or click its green maximize button, and that window leaves the group on the spot — before anything resizes. That ordering is the point: a maximize is a resize like any other as far as macOS is concerned, so left alone it would drag the mates along a shared edge and squish them to the screen edges, and un-maximizing would not give their sizes back. Leaving first means the other members are never touched and keep exactly the sizes you gave them. They also stay grouped with *each other* — only the window you maximized drops out (and a two-member group simply dissolves, since one window is not a group). To put it back, mate it again the normal way: drag it until its edge nears a member's facing edge and release.
