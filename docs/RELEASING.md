...
## Regenerating the site diagrams

`docs/index.html` embeds layout diagrams generated by Ferrite's own
`GroupLayoutSolver`, so a diagram on the site cannot depict an arrangement the
app would not produce (the same principle as `PresetGlyph` in the menu).

```sh
mkdir -p /tmp/gensvg && cp scripts/gen-site-diagrams.swift /tmp/gensvg/main.swift
swiftc -O Sources/FerriteCore/GroupLayoutSolver.swift /tmp/gensvg/main.swift -o /tmp/gensvg/gen
/tmp/gensvg/gen
```

It prints one `<svg>` per preset, delimited by `<!--id-->` / `<!--label:...-->`
comments; paste the ones that changed into the corresponding `<figure>` blocks.
The generator compiles the solver source directly (it is pure geometry with no
AppKit dependency), so it needs no SwiftPM target and adds nothing to the
package. Re-run it whenever a preset's geometry changes.
