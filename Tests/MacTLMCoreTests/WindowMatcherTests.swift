...
final class WindowMatcherTests: XCTestCase {
    /// Stand-in for the app layer's salted hash: same title -> same identity.
    /// Core never hashes, so the tests supply the mapping themselves.
    private func hash(_ title: String) -> String { "h:\(title)" }

    private func record(slot: Int, title: String, pin: String? = nil) -> WindowRecord {
        WindowRecord(slot: slot, titleHash: hash(title),
                     frame: NormalizedFrame(x: Double(slot) * 0.1, y: 0, w: 0.2, h: 0.9),
                     pinPattern: pin, lastSeen: Date(timeIntervalSince1970: 0))
    }

    private func candidate(id: Int, title: String, order: Int) -> WindowCandidate {
        WindowCandidate(id: id, title: title, titleHash: hash(title), order: order)
    }

    func testOrderFallbackWhenTitlesAllChanged() {
        let records = [record(slot: 0, title: "Old A"), record(slot: 1, title: "Old B")]
        let windows = [candidate(id: 10, title: "New X", order: 0),
                       candidate(id: 11, title: "New Y", order: 1)]
    ...
    func testExactTitleMatchBeatsOrder() {
        ...
        // Reopened in swapped order — identity hashes must win.
        let windows = [candidate(id: 10, title: "Personal", order: 0),
                       candidate(id: 11, title: "Work", order: 1)]
        let result = WindowMatcher.assign(records: records, to: windows)
    ...
    func testPinPatternBeatsExactTitle() {
        ...
        // exact old identity of record 1. Pin claims first.
        let records = [record(slot: 0, title: "Anything", pin: "Work"),
                       record(slot: 1, title: "Work — Arc")]
        let windows = [candidate(id: 10, title: "Work — Arc", order: 0),
                       candidate(id: 11, title: "My Work Tabs", order: 1)]
        let result = WindowMatcher.assign(records: records, to: windows)
    ...
    func testExtraWindowsAreLeftUnassigned() {
        ...
        let windows = [candidate(id: 10, title: "Only", order: 0),
                       candidate(id: 11, title: "Extra", order: 1)]
        let result = WindowMatcher.assign(records: records, to: windows)
    ...
    func testFewerWindowsThanRecordsAssignsSubset() {
        ...
        let windows = [candidate(id: 10, title: "B", order: 0)]
        let result = WindowMatcher.assign(records: records, to: windows)
        XCTAssertEqual(result[10]?.slot, 1)
        XCTAssertEqual(result.count, 1)
    }
    ...
    func testInvalidPinRegexIsIgnoredNotCrashing() {
        ...
        let windows = [candidate(id: 10, title: "A", order: 0)]
        let result = WindowMatcher.assign(records: records, to: windows)
        XCTAssertEqual(result[10]?.slot, 0, "falls back to hash/order matching")
    }
    ...
    func testPinnedRecordWithNoMatchingWindowFallsThrough() {
        ...
        let windows = [candidate(id: 10, title: "B", order: 0),
                       candidate(id: 11, title: "A", order: 1)]
        let result = WindowMatcher.assign(records: records, to: windows)
        XCTAssertEqual(result[10]?.slot, 1)
        XCTAssertEqual(result[11]?.slot, 0)
    }
    ...
    func testDuplicateIdenticalRecordsBothAssign() {
        ...
        let windows = [candidate(id: 10, title: "X", order: 0),
                       candidate(id: 11, title: "Y", order: 1)]
        let result = WindowMatcher.assign(records: records, to: windows)
    ...
    func testOrderFallbackDisabledLeavesUnmatchedWindowsAlone() {
        let records = [record(slot: 0, title: "Old A"), record(slot: 1, title: "Old B")]
        let windows = [candidate(id: 10, title: "Open", order: 0)]
        let result = WindowMatcher.assign(records: records, to: windows,
                                          allowOrderFallback: false)
        XCTAssertTrue(result.isEmpty, "no hash match and no fallback -> nothing assigned")
    }
